...
function GATSP(param){
	...
	this.calculate_fitness = function(){
		...
		if(this.max_dist - this.min_dist <= this.ex_fitness){//当前的最有个体的适应度小于等于父辈最优个体的适应度
			this.continuous_num++;
		}else{
			this.continuous_num = 0;
		}
	...
	this.hybridization = function(p1,p2,p3,c){
		...
		switch(Math.floor(Math.random()*3)){
			case 0: c.path = p1.path.clone(); break;
			case 1: c.path = p2.path.clone(); break;
			case 2: c.path = p3.path.clone(); break;
		}
		//根据杂交率决定是否杂交
